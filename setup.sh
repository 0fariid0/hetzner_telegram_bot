...
configure_projects_interactive() {
  ...
  read -rp "Number of Hetzner projects [1]: " count
  count="${count:-1}"
  ...
  for ((i=1; i<=count; i++)); do
    echo >&2
    read -rp "Project ${i} display name [Project ${i}]: " name
    name="${name:-Project ${i}}"
    name="${name//$'\t'/ }"
    name="${name//$'\n'/ }"
    read -rsp "Hetzner API Token for ${name} (Read/Write): " token
    echo >&2
...
install_flow() {
  ...
  if is_installed; then
    ...
    read -rp "Reinstall and replace configuration? [y/N]: " answer
  ...
  echo
  read -rp "Allowed Telegram numeric User ID: " allowed_user
...
add_project_token() {
  ...
  local name token
  echo
  read -rp "New project display name: " name
  name="${name//$'\t'/ }"
  name="${name//$'\n'/ }"
  ...
  read -rsp "Hetzner API Token (Read/Write): " token
  echo
...
change_project_token() {
  ...
  echo
  read -rp "Project display name [${name}]: " new_name
  new_name="${new_name:-$name}"
  new_name="${new_name//$'\t'/ }"
  new_name="${new_name//$'\n'/ }"
  read -rsp "New Hetzner API Token for ${new_name}: " new_token
  echo
...
remove_project_token() {
  ...
  read -rp "Remove project '${name}'? [y/N]: " answer
...
change_telegram_token() {
  ...
  local token
  echo
  read -rsp "New Telegram Bot Token: " token
  echo
...
change_allowed_user() {
  ...
  local user_id
  echo
  read -rp "New allowed Telegram numeric User ID: " user_id
